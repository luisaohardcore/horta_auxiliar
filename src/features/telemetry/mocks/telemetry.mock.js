...
/**
 * Generates 7 days of hourly readings for a given canteiro.
 * edge param controls special scenarios per canteiro.
 */
export const generateTelemetryForCanteiro = (canteiroId, edge = 'normal') => {
  ...
  for (let i = HOURS; i >= 0; i--) {
    ...
    const peak = 13;
    const lux  = isDay
      ? Math.round(Math.max(0, 130000 - Math.abs(hour - peak) * 18000 + (Math.random() * 10000 - 5000)))
      : 0;

    // Force pH spike (suspicious reading) on canteiro-a at day 3, noon
...
// Backward-compat (existing tests rely on these)
